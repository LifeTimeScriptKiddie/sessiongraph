...
def _agentctl_quality(events: list[Event]) -> list[Finding]:
    findings: list[Finding] = []
    if not events or events[-1].kind != "loop_finish" or events[-1].metadata.get("status") not in {
        "passed", "stopped", "paused", "failed",
    }:
        findings.append(Finding(
            "loop_incomplete", "warning", "No recognized terminal record at the end of the selected loop trace",
            [events[-1].id] if events else [],
            "Capture the terminal record or label this as an in-progress/partial trace. "
            "Do not treat fewer recorded events as a completed workflow improvement.",
        ))
    unclassified = [event for event in events
        if event.kind in {"loop_generate", "loop_evaluate"}
        and event.metadata.get("ok") is False
        and (not isinstance(event.metadata.get("failureClass"), str)
             or event.metadata["failureClass"].strip().lower() in {"", "none", "unknown"})]
    if unclassified:
        findings.append(Finding(
            "loop_telemetry_gap", "warning", f"Failure classification missing for {len(unclassified)} agent stage(s)",
...
def _parents(event: Event) -> list[str]:
    return event.metadata.get("parent_ids", [event.parent_id] if event.parent_id else [])


def _relation(event: Event, parent_id: str) -> str:
    return event.metadata.get("parent_relations", {}).get(parent_id, "precedes")
...
def analyze(session: Session) -> dict[str, Any]:
    ...
    return {
        ...
        "metrics": {
            "events": len(events), "edges": sum(len(_parents(event)) for event in events),
            ...
            **({
                ...
                "loop_terminal_recorded": bool(events and events[-1].kind == "loop_finish"
                    and events[-1].metadata.get("status") in {"passed", "stopped", "paused", "failed"}),
                "loop_failure_classification_missing": sum(
                    event.kind in {"loop_generate", "loop_evaluate"}
                    and event.metadata.get("ok") is False
                    and (not isinstance(event.metadata.get("failureClass"), str)
                         or event.metadata["failureClass"].strip().lower() in {"", "none", "unknown"})
                    for event in events),
        ...
        "graph": {
            ...
            "edges": [{"from": parent_id, "to": event.id, "relation": _relation(event, parent_id)}
...
def compare(before: dict[str, Any], after: dict[str, Any]) -> dict[str, Any]:
    ...
    keys.extend(
        key for key in ("merge_events", "loop_iterations", "loop_retries", "loop_duration_ms",
