...
def mermaid(analysis: dict[str, Any]) -> str:
    ...
    for edge in analysis["graph"]["edges"]:
        relation = _mermaid_label(str(edge.get("relation") or "precedes")).replace("|", "&#124;")
        lines.append(
            f'  {_mermaid_id(edge["from"])} -->|"{relation}"| {_mermaid_id(edge["to"])}'
        )
    return "\n".join(lines) + "\n"
...
def markdown(analysis: dict[str, Any]) -> str:
    ...
    if "loop_iterations" in metrics:
        lines[4:4] = [
            ...
            f"- Terminal record: {metrics.get('loop_terminal_recorded', 'unknown')}; "
            f"stage failures without classification: {metrics.get('loop_failure_classification_missing', 'unknown')}",
        ]
